...
Entity::Entity(Vector2<float> position, Vector2<int> size)
    : Object2D(position, size), m_collision_enabled(true) {}

void Entity::physics_process(
    float delta_time, const std::vector<std::unique_ptr<Entity>> &colliders) {
...
void Entity::move(const Vector2<float> &offset) {
...
const SDL_Rect Entity::get_collision_bounds() const { return get_transform(); }
