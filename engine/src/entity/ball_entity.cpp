...
BallEntity::BallEntity(float speed, Vector2<float> position, Vector2<int> size)
    : Engine::Entity(position, size),
      m_movement_direction(Vector2<float>{-1, -1}.normalized()), m_speed(speed),
...
void BallEntity::physics_process(
    float delta_time,
    const std::vector<std::unique_ptr<Engine::Entity>> &colliders) {
  ...
  Vector2<float> movement_offset = m_movement_direction * m_speed * delta_time;
...
void BallEntity::on_collision(Engine::Entity *entity) {
  ...
  Vector2<int> entity_center = {collider_bounds.x + collider_bounds.w / 2,
                                collider_bounds.y + collider_bounds.h / 2};
  ...
  std::cout << "col[ pos:" << entity->get_position()
